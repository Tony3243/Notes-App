...
export async function login(req, res) {
    ...
    const {data: nonexistentUser, error} = await supabase.from('users').select('email, password_hash').eq('email', email).single(); //selects email form users database
    ...
    jwt.sign({userId: nonexistentUser.id, email: nonexistentUser.email}, process.env.JWT_SECRET, {expiresIn: "1h"}, (err, token) => {
        if(err) {
